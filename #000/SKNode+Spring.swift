        ...
        let animation = SKAction.customAction(withDuration: duration) {
            ...
            if elapsedTime < CGFloat(duration) {
                ...
                if dampingRatio < 1 {
                    ...
                    let expNum: CGFloat = exp(-dampingRatio * naturalFrequency! * elapsedTime)
                    let cosNum: CGFloat = cos(dampedFrequency! * elapsedTime)
                    let sinNum: CGFloat = sin(dampedFrequency! * elapsedTime)
                    currentValue = finalValue! - expNum * (A! * cosNum + B! * sinNum)
                } else if dampingRatio == 1 {

                    A = A ?? initialDistance!
                    B = B ?? (naturalFrequency! - velocity) * initialDistance!
                    let expNum: CGFloat = exp(-dampingRatio * naturalFrequency! * elapsedTime)
                    currentValue = finalValue! - expNum * (A! + B! * elapsedTime)
                } else {
